...
@section('content')
	          	...
	          	<div class="modal-body">
		            ...
		            <div class="row">
		            	...
		            	<div class="col-md-3">
			                ...
			                <div class="col-md-12">
								<select id="Status" class="form-control select2" style="width: 100%; font-weight: normal;" disabled>
		            ...
		            <div class="row">
		            	<div class="col-md-12">
			                ...
			                <div class="col-md-12">
	                            ...
	                            <input type="text" data-type="Member" id="WithdrawByMember" name="WithdrawByMember" placeholder="Member Name" class="form-control" readonly>
			                </div>
		            	</div>
		            </div>
		            ...
		            <div id="divCheckInfo" class="row">
		            	<div class="col-md-4">
			                ...
			                <div class="col-md-12">
			                    <input id="CheckNo" type="text" class="form-control" value="" style="width:100%; font-weight:normal;" readonly>
			                </div>
		            	</div>
		            	<div class="col-md-4">
			                <label class="col-md-12" style="font-weight: normal;">Check Date <span style="color:red;">*</span></label>
			                <div class="col-md-12">
			                    <input id="CheckDate" type="text" class="form-control" value="" style="width:100%; font-weight:normal;" readonly>
                            </div>
		            	</div>
		            	<div class="col-md-4">
		            ...
		            <div id="divSendThroughInfo" class="row" style="display: none;">
		            	...
		            	<div class="col-md-8">
			                ...
			                <div class="col-md-12">
			                    <input id="SenderName" type="text" class="form-control" value="" style="width:100%; font-weight:normal;" readonly>
			                </div>
		            	</div>
		            	<div class="col-md-4">
			                <label class="col-md-12" style="font-weight: normal;">Control No./Reference No.<span style="color:red;">*</span></label>
			                <div class="col-md-12">
			                    <input id="SendingRefNo" type="text" class="form-control" value="" style="width:100%; font-weight:normal;" readonly>
			                </div>
		            	</div>
		            </div>
		            <div style="clear:both;"></div>
				            ...
				            <div class="row">
				            	<div class="col-md-12">
									<div class="input-group margin pull-right">
										...
										<input id="ApprovedAmount" type="text" placeholder="Approved Amount" class="form-control DecimalOnly RecomputeTotal" style="width:100%; font-weight:normal; text-align: right;" readonly>
									</div>		
								</div>		
							</div>	
				            <div class="row">
				            	<div class="col-md-12">
									<div class="input-group margin pull-right">
										...
										<input id="ProcessingFee" type="text" placeholder="Processing Fee" class="form-control DecimalOnly RecomputeTotal" style="width:100%; font-weight:normal; text-align: right;" readonly>
									</div>		
								</div>	
							</div>	
				            <div class="row">
	...
	<script type="text/javascript">
			...
			tdOption = " <div class='input-group-btn' style='padding-left:10px; float:left; display: inline-block;'>" +
	                        ...
	                        "<ul class='dropdown-menu'> ";
		                  	tdOption += " <li style='text-align:left;'> " +
		                      	" <a href='#' onclick='EditRecord(" + vData.WithdrawalID + "," + (vData.Status == '{{ config('app.STATUS_PENDING') }}' ? true : false) + ")'>" + 
		                      		" <strong><i class='fa fa-info-circle font-size:15px;'></i> " + (vData.Status == '{{ config('app.STATUS_PENDING') }}' ? "Edit Withdrawal" : "View Withdrawal") + "</strong>" +
		                      	" </a> " +
		                   	" </li> ";
            tdOption += " </ul> " +
	                    " </div> " ;
	    ...
	    function Clearfields(){
			...
			$("#WithdrawByMemberID").val('{{ Session('MEMBER_ID') }}');
			$("#WithdrawByMember").val('{{ Session('MEMBER_NAME') }}');
			$("#WithdrawByMemberMobileNo").val('{{ Session('MEMBER_MOBILE_NO') }}');
			$("#WithdrawByMemberEmailAddress").val('{{ Session('MEMBER_EMAIL_ADDRESS') }}');
			...
			$("#SendToFirstName").val('{{ Session('MEMBER_FIRSTNAME') }}');
			$("#SendToLastName").val('{{ Session('MEMBER_LASTNAME') }}');
			$("#SendToMiddleName").val('{{ Session('MEMBER_MIDDLENAME') }}');
			$("#SendToTelNo").val('{{ Session('MEMBER_TEL_NO') }}');
			$("#SendToMobileNo").val('{{ Session('MEMBER_MOBILE_NO') }}');
			$("#SendToEmailAddress").val('{{ Session('MEMBER_EMAIL_ADDRESS') }}');
			$("#SenderName").val('');
			$("#SendingRefNo").val('');
	    ...
	    function NewRecord(){
			...
			$.ajax({
				...
				url: "{{ route('get-member-ewallet-balance') }}",
				dataType: "json",
				success: function(data){
					...
					if(data.Response =='Success'){
				    	isNewRecord = 1;
						Clearfields();
						$("#EWalletBalance").val(FormatDecimal(data.EwalletBalance,2));
						$("#record-info-modal").modal();
					}else{
						showJSModalMessageJS("New Withdrawal",data.ResponseMessage,"OK");
					}
				},
				error: function(data){
			        $("#divLoader").hide();
					...
					success: function(data){
						...
						if(data.Response =='Success' && data.EWalletWithdrawalInfo != undefined){
							...
							if(data.EWalletWithdrawalInfo.WithdrawalOption == "Check"){
								...
								$("#Bank").val(data.EWalletWithdrawalInfo.Bank);
							...
							if(data.EWalletWithdrawalInfo.Status == "{{ config('app.STATUS_APPROVED') }}"){
	    ...
	    function SaveRecord(){
			...
			}else if(EWalletBalance < RequestedAmount) {
				...
				$.ajax({
					...
					success: function(data){
						...
						if(data.Response =='Success'){
							...
							showJSModalMessageJS("Save Encashment",data.ResponseMessage,"OK");
						}
					},
					error: function(data){
				        $("#divLoader").hide();
