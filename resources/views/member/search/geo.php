...
<div class="box">
    ...
    <script type="text/ecmascript">
        var nodes = [
            ...
            @if(!File::exists('img/members/'.$TOP - > EntryID.
                '.jpg'))
            <?php
            ...
            ?>
            @if(!File::exists('img/members/'.$child - > EntryID.
                '.jpg'))
            <?php
            $ProfilePic = $PicPath . "member-no-image.png";
            ?>
    ...
    <style type="text/css">
        @if(isset($TOP)) @if($TOP->PackageID ==1) g[data-node-id='1']>path {
            fill: #9a6941 !important;
            stroke: #d7b1a0 !important;
        }

        @elseif($TOP->PackageID ==2) g[data-node-id='1']>path {
            fill: #a2a2a1 !important;
            stroke: #dfddd6 !important;
        }

        @elseif($TOP->PackageID ==3) g[data-node-id='1']>path {
            fill: #dfb407 !important;
            stroke: #f8e692 !important;
        }

        @else g[data-node-id='1']>path {
            fill: #96856e !important;
            stroke: #f8bbc3 !important;
        }

        @endif @endif @foreach($TREE as $child) @if($child->PackageID ==1) g[data-node-id='{{ $child->Cntr }}']>path {
            fill: #9a6941 !important;
            stroke: #d7b1a0 !important;
        }

        @elseif($child->PackageID ==2) g[data-node-id='{{ $child->Cntr }}']>path {
            fill: #a2a2a1 !important;
            stroke: #dfddd6 !important;
        }

        @elseif($child->PackageID ==3) g[data-node-id='{{ $child->Cntr }}']>path {
            fill: #dfb407 !important;
            stroke: #f8e692 !important;
        }

        @else g[data-node-id='{{ $child->Cntr }}']>path {
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="col-md-12" style="font-weight: normal;">
                                <select id="City" class="form-control select2" style="width: 100%; height: 100px; font-weight: normal;">
                                    ...
                                    <option value="{{ $ckey->CityID }}" data-cityid="{{$ckey->CityID}}" data-cityprovince="{{$ckey->Province}}" data-cityzipcode="{{$ckey->ZipCode}}">{{ $ckey->City }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
