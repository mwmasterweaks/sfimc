...
@section('content')
	...
	<div id="set-as-paid-modal" class="modal fade bs-example-modal-lg" style="display:none;"> 
	  <div class="modal-dialog modal-lg">
	    <div class="modal-content">

	          <div class="modal-header" style="background-color: #3c8dbc;">
	            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
	            <h4 class="modal-title"  style="text-align:center; color:white;"><span><b>Order - Set As Paid</b></span></h4>
	          </div>

	          <div class="modal-body">

	        	<input type="hidden" id="PaidOrderID" value="0" readonly>

	            <div class="row">
	            	<div class="col-md-12">
		                <label class="col-md-12" style="font-weight: normal;">Are you sure you want to set this order as paid?</label>
	            	</div>
            	</div>
	            ...
	            <div class="modal-footer">
					<div class="input-group pull-right">
						<span class="input-group-btn">
							<a id="btnSavePaid" href="#" class="btn btn-info btn-flat" onclick="ProceedPaidOrder()"><i class="fa fa-save"></i> Proceed</a>
							<a href="#" class="btn btn-danger btn-flat" data-dismiss="modal" aria-label="Close"><i class="fa fa-remove"></i> Cancel</a>
						</span>
					</div>	
	            </div>
	...
	<div id="set-as-cancelled-modal" class="modal fade bs-example-modal-lg" style="display:none;"> 
	  	<div class="modal-dialog modal-lg">
	    	<div class="modal-content">
	          	<div class="modal-header" style="background-color: #3c8dbc;">
	            	<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
	            	<h4 class="modal-title"  style="text-align:center; color:white;"><span><b>Cancel Order</b></span></h4>
	          	</div>

          		<div class="modal-body">
        			<input type="hidden" id="CancelledOrderID" value="0" readonly>
		            <div class="row">
		            	<div class="col-md-12">
			                <label class="col-md-12" style="font-weight: normal;">Are you sure you want to cancel this order?</label>
		            	</div>
	            	</div>
		            <div style="clear:both;"></div>
		            <br>                
		            ...
		            <div class="modal-footer">
						<div class="input-group pull-right">
							<span class="input-group-btn">
								<a id="btnSaveCancel" href="#" class="btn btn-info btn-flat" onclick="ProceedCancelOrder()"><i class="fa fa-save"></i> Proceed</a>
								<a href="#" class="btn btn-danger btn-flat" data-dismiss="modal" aria-label="Close"><i class="fa fa-remove"></i> Cancel</a>
							</span>
						</div>	
		            </div>
	...
	<script type="text/javascript">
			...
			tdOption = " <div class='input-group-btn' style='padding-left:10px; float:left; display: inline-block;'>" +
	                        ...
	                        "<ul class='dropdown-menu'> ";
		                  	tdOption += " <li style='text-align:left;'> " +
		                   	...
		                   	" </li> ";
		                  	tdOption += " <li style='text-align:left;'> " +
	                              	" <a href='{{ route('admin-order-print') }}?OrderID=" + vData.OrderID + "' target='blank_' >" + 
	                              		" <strong><i class='fa fa-info-circle'font-size:15px;'></i> Print " + 
	                              		" </strong>" +
	                              	" </a> " +
	                           	" </li> ";

			        if(vData.IsPaid != 1){
			        	if(vData.Status != '{{ config('app.STATUS_CANCELLED') }}' && vData.Status != '{{ config('app.STATUS_RETURNED') }}'){
	                          	tdOption += " <li style='text-align:left;'> " +
	                              	" <a href='#' onclick='SetAsPaid(" + vData.OrderID + ")'>" + 
	                              		" <strong><i class='fa fa-minus-circle' style='font-size:15px;'></i> Set As Paid " + 
	                              		" </strong>" +
	                              	" </a> " +
	                           	" </li> ";
	                    }
			        ...
			        if(vData.IsPaid != 1){
			        	if(vData.Status != '{{ config('app.STATUS_CANCELLED') }}' && vData.Status != '{{ config('app.STATUS_RETURNED') }}' && vData.Status != '{{ config('app.STATUS_DELIVERED') }}'){
		                  	tdOption +=
		                      	" <li style='text-align:left;'> " +
		                          	" <a href='#' onclick='CancelOrder(" + vData.OrderID + ")'>" + 
		                          		" <strong><i class='fa fa-close font-size:15px;'></i> Cancel Order " + 
		                          		" </strong>" +
		                          	" </a> " +
		                       	" </li> ";
				        }
	    ...
	    function SetAsPaid(vOrderID){

	    	if(vOrderID > 0){

				$("#PaidOrderID").val(vOrderID);
				$("#set-as-paid-modal").modal();

	    	}

	    }

	    function ProceedPaidOrder(){

			$.ajax({
				type: "post",
				data: {
					_token: '{{ $Token }}',
					OrderID: $("#PaidOrderID").val()
				},
				url: "{{ route('do-paid-order') }}",
				dataType: "json",
				success: function(data){
					buttonOneClick("btnSavePaid", "Proceed", false);

					if(data.Response =='Success'){
						$("#set-as-paid-modal").modal('hide');
						showMessage("Success",data.ResponseMessage);
						LoadRecordRow(data.OrderInfo);
					}else{
						showJSModalMessageJS("Order - Set As Paid",data.ResponseMessage,"OK");
					}
				},
				error: function(data){
					buttonOneClick("btnSavePaid", "Proceed", false);
				...
				beforeSend:function(vData){
					buttonOneClick("btnSavePaid", "", true);
				}
        	});

	    };

	    function CancelOrder(vOrderID){
	    	if(vOrderID > 0){
				$("#CancelledOrderID").val(vOrderID);
	    ...
	    function ProceedCancelOrder(){
	    	...
	    	if($("#CancellationReason").val() == ""){
				showJSModalMessageJS("Cancel Order","Please enter the reason for the cancellation of this order.","OK");
	    	}else{
				$.ajax({
					type: "post",
					data: {
						_token: '{{ $Token }}',
						OrderID: $("#CancelledOrderID").val(),
						Reason : $("#CancellationReason").val()
					},
					url: "{{ route('do-cancel-order') }}",
					dataType: "json",
					success: function(data){
				        $("#divLoader").hide();
						...
						if(data.Response =='Success'){
							...
							showJSModalMessageJS("Order Cancellation",data.ResponseMessage,"OK");
						}
					},
					error: function(data){
				        $("#divLoader").hide();
