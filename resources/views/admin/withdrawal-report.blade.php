		              	...
		              	<table id="tblList" class="table table-bordered table-hover" style="overflow-x:auto;">
			                <thead>
				                <tr>
				                  ...
				                  <th>IBO Number</th>
				                  <th>Member Name</th>
				                  <th style="text-align: right;">Total Withdrawal</th>
				                </tr>
			                </thead>
			                <tbody>
				            </tbody>
	...
	<script type="text/javascript">
	    ...
	    function getRecordList(vPageNo){
			...
			$.ajax({
				...
				url: "{{ route('get-withdrawal-report') }}",
				dataType: "json",
				success: function(data){
					LoadRecordList(data.WithdrawalList);
			        $("#divLoader").hide();
				},
				error: function(data){
	    ...
	    function LoadRecordRow(vData){
			...
			tdTotalApprovedAmount = "<span style='font-weight:normal;' class='pull-right'>" + FormatDecimal(vData.TotalApprovedAmount,2) + "</span>";
			...
			tblList.rows().every(function ( rowIdx, tableLoop, rowLoop ) {
			    ...
			    if(rowData[0] == vData.MemberID){
			    	...
			    	curData[3] = tdTotalApprovedAmount;
