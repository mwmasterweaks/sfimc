...
@section('content')
	...
	<div id="set-delivered-modal" class="modal fade bs-example-modal-lg" style="display:none;"> 
	  <div class="modal-dialog modal-lg">
	    <div class="modal-content">

	          <div class="modal-header" style="background-color: #3c8dbc;">
	            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
	            <h4 class="modal-title"  style="text-align:center; color:white;"><span><b>Order - Set As Delivered</b></span></h4>
	          </div>

	          <div class="modal-body">

	        	<input type="hidden" id="DeliveredOrderID" value="0" readonly>

	            <div class="row">
	            	<div class="col-md-12">
		                <label class="col-md-12" style="font-weight: normal;">Are you sure you want to set this order as delivered?</label>
	            	</div>
            	</div>
	            <div style="clear:both;"></div>
	            <br>                
	            <div class="modal-footer">
					<div class="input-group pull-right">
						<span class="input-group-btn">
							<a id="btnDeliveredOrder" href="#" class="btn btn-info btn-flat" onclick="ProceedDeliveredOrder()"><i class="fa fa-save"></i> Proceed</a>
							<a href="#" class="btn btn-danger btn-flat" data-dismiss="modal" aria-label="Close"><i class="fa fa-remove"></i> Cancel</a>
						</span>
					</div>	
	            </div>
	...
	<script type="text/javascript">
			...
			tdOption = " <div class='input-group-btn' style='padding-left:10px; float:left; display: inline-block;'>" +
	                        ...
	                        "<ul class='dropdown-menu'> ";
		                  	tdOption += " <li style='text-align:left;'> " +
		                   	...
		                   	" </li> ";
                      	tdOption += " <li style='text-align:left;'> " +
                          	" <a href='#' onclick='SetAsDelivered(" + vData.OrderID + ")'>" + 
                          		" <strong><i class='fa fa-minus-circle' style='font-size:15px;'></i> Set As Delivered " + 
                          		" </strong>" +
                          	" </a> " +
                       	" </li> ";
	    ...
	    function SetAsDelivered(vOrderID){

	    	if(vOrderID > 0){

				$("#DeliveredOrderID").val(vOrderID);
				$("#set-delivered-modal").modal();

	    	}

	    }

	    function ProceedDeliveredOrder(){

			$.ajax({
				type: "post",
				data: {
					_token: '{{ $Token }}',
					OrderID: $("#DeliveredOrderID").val()
				},
				url: "{{ route('set-as-delivered-order') }}",
				dataType: "json",
				success: function(data){
					buttonOneClick("btnDeliveredOrder", "Proceed", false);

					if(data.Response =='Success'){
						$("#set-delivered-modal").modal('hide');
						...
						showJSModalMessageJS("Delivered Order",data.ResponseMessage,"OK");
					}
				},
				error: function(data){
					buttonOneClick("btnDeliveredOrder", "Proceed", false);
				...
				beforeSend:function(vData){
					buttonOneClick("btnDeliveredOrder", "", true);
				}
        	});
