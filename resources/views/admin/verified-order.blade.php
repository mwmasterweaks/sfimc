...
@section('content')
	...
	<section class="content-header">
		...
		<ol class="breadcrumb">
			...
			<li class="active">Verified Order</li>
		</ol>
	</section>
	...
	<div id="set-packed-modal" class="modal fade bs-example-modal-lg" style="display:none;"> 
	  <div class="modal-dialog modal-lg">
	    <div class="modal-content">

	          <div class="modal-header" style="background-color: #3c8dbc;">
	            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
	            <h4 class="modal-title"  style="text-align:center; color:white;"><span><b>Order - Set As Packed</b></span></h4>
	          </div>

	          <div class="modal-body">

	        	<input type="hidden" id="PackedOrderID" value="0" readonly>

	            <div class="row">
	            	<div class="col-md-12">
		                <label class="col-md-12" style="font-weight: normal;">Are you sure you want to set this order as packed?</label>
	            	</div>
            	</div>
	            ...
	            <div class="modal-footer">
					<div class="input-group pull-right">
						<span class="input-group-btn">
							<a id="btnPackedOrder" href="#" class="btn btn-info btn-flat" onclick="ProceedPackedOrder()"><i class="fa fa-save"></i> Proceed</a>
							<a href="#" class="btn btn-danger btn-flat" data-dismiss="modal" aria-label="Close"><i class="fa fa-remove"></i> Cancel</a>
						</span>
					</div>	
	            </div>
	...
	<script type="text/javascript">
			...
			tdOption = " <div class='input-group-btn' style='padding-left:10px; float:left; display: inline-block;'>" +
	                        ...
	                        "<ul class='dropdown-menu'> ";
		                  	tdOption += " <li style='text-align:left;'> " +
		                   	...
		                   	" </li> ";
                      	tdOption += " <li style='text-align:left;'> " +
                          	" <a href='#' onclick='SetAsPacked(" + vData.OrderID + ")'>" + 
                          		" <strong><i class='fa fa-minus-circle' style='font-size:15px;'></i> Set As PAcked " + 
                          		" </strong>" +
                          	" </a> " +
                       	" </li> ";
	    ...
	    function SetAsPacked(vOrderID){

	    	if(vOrderID > 0){

				$("#PackedOrderID").val(vOrderID);
				$("#set-packed-modal").modal();

	    	}

	    }

	    function ProceedPackedOrder(){

			$.ajax({
				type: "post",
				data: {
					_token: '{{ $Token }}',
					OrderID: $("#PackedOrderID").val()
				},
				url: "{{ route('do-packed-order') }}",
				dataType: "json",
				success: function(data){
					buttonOneClick("btnPackedOrder", "Proceed", false);

					if(data.Response =='Success'){
						$("#set-packed-modal").modal('hide');
						...
						showJSModalMessageJS("Packed Order",data.ResponseMessage,"OK");
					}
				},
				error: function(data){
					buttonOneClick("btnPackedOrder", "Proceed", false);
				...
				beforeSend:function(vData){
					buttonOneClick("btnPackedOrder", "", true);
				}
        	});
