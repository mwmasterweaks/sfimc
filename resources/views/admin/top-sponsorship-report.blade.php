...
@section('content')
	...
	<section class="content-header">
		<h1>
			Top Sponsorship Report
		</h1>
		<ol class="breadcrumb">
			<li><a href="{{ route('admin-dashboard') }}">Home</a></li>
			<li class="active">Top Sponsorship Report</li>
		</ol>
	</section>
		              	...
		              	<table id="tblList" class="table table-bordered table-hover" style="overflow-x:auto;">
			                <thead>
				                <tr>
				                  ...
				                  <th>IBO Number</th>
				                  <th>Member Name</th>
				                  <th style="text-align: right;">Total Sponsored</th>
				                </tr>
			                </thead>
			                <tbody>
				            </tbody>
	...
	<script type="text/javascript">
	    ...
	    function getRecordList(vPageNo){
			...
			$.ajax({
				...
				url: "{{ route('get-sponsorship-report') }}",
				dataType: "json",
				success: function(data){
					LoadRecordList(data.SponsorshipList);
			        $("#divLoader").hide();
				},
				error: function(data){
	    ...
	    function LoadRecordRow(vData){
			...
			tdTotalSponsoredCount = "<span style='font-weight:normal;' class='pull-right'>" + FormatDecimal(vData.TotalSponsoredCount,0) + "</span>";
			...
			tblList.rows().every(function ( rowIdx, tableLoop, rowLoop ) {
			    ...
			    if(rowData[0] == vData.MemberID){
			    	...
			    	curData[3] = tdTotalSponsoredCount;
